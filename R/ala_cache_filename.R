...
ala_cache_filename <- function(url) {
    ...
    if (! is.null(this_url$query)) {
        ...
        if (! is.null(this_url$query$fl)) {
            this_url$query$fl <-
                str_c(sort(str_split(this_url$query$fl, ",")[[1]]),
                      collapse = ",")
        }
        ## Note that this means that the ordering of columns coming back
        ## from the query may not match the ordering that the user provided
        ## the list of fields in. Need to decide if this is acceptable
    }
    url <- build_url(this_url) ## reconstruct URL string

    ## use md5 hash of url as the cache filename, because it's an easy way of
    ## generating unique filenames based on the URL
    x <- digest(url)
    ## the downside is that we can't easily go from a cache filename back to
    ## its URL
